...
AI Agent를 통하여 자연어로 편하게 독서기록장을 관린하는 기능 구현.
이를 위하여 AI Agent에세 Skill을 입력하여 개인화된 독서 관리 가능.
...
- 주요 기능
  ...
  - 독서 기록부에 감상문 올리는 기능 
- 착안점
  - AI 비용 절감을 위해 경량화 모델 사용 가능
  - AI 기능 사용시 AI API 호출 최소화
...
| 단계 | 담당 | 입력 | 출력 | 설명 |
|------|------|------|------|------|
| 1 | Telegram | 입력 | ISBN 입력 | 사용자가 ISBN을 입력해서 전송 |
| 2 | Brave Search | 도서 검색 | 도서 검색 결과 | ISBN으로 도서 정보 검색 (Google Books, 예스24 등) |
| 3 | Gemini API | 검색 결과 | 정제된 메타데이터 | 검색 결과에서 중요 정보 추출 및 요약 |
| 4 | Notion API | 메타데이터 | Notion DB 저장 | 추출된 정보를 개인 Notion 라이브러리에 저장 |
| 5 | Google Chat | 완료 메시지 | 사용자 피드백 | 저장 완료 알림 및 링크 전송 |
